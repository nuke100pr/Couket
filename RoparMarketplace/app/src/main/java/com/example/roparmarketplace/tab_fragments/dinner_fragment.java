...
public class dinner_fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_dinner_fragment, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView = view.findViewById(R.id.rvDinnerList);
        progressBar = view.findViewById(R.id.dinner_list_progress_bar);
                                    ...
                                    for(DataSnapshot ds :snapshot.getChildren() ){
                                        ...
                                        if(ds.exists())
                                        {
                                            databaseReference.child(userId).child(requestId).addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                    if(snapshot.child(nodenames.COUPON_TYPE).getValue().toString().trim().equals("Kanaka-dinner")||snapshot.child(nodenames.COUPON_TYPE).getValue().toString().trim().equals("Bhopal-dinner"))
                                                    {
                                                        if((snapshot.exists())) {
